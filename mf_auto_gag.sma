...
enum {
    MATCH_EXACT = 0,   // kelime
    MATCH_PREFIX,      // kelime*
    MATCH_SUFFIX,      // *kelime
    MATCH_CONTAINS     // *kelime*
};

new Trie:g_tBadWords;
new Array:g_aBadWords;
new Array:g_aBadRoots;
new Array:g_aBadMatchType;
...
bool:CheckWordMatch(const szWord[]);
bool:ContainsSubsequence(const szText[], const szPattern[], maxGap, maxStart);
...
public plugin_init() {
    ...
    g_pCvarImmunity = create_cvar("amx_autogag_immunity", "0"); // 0 = Yetkililer de denetlenir (Test edilebilir), 1 = Yetkililer muaf
    
    g_tBadWords = TrieCreate();
    g_aBadWords = ArrayCreate(64);
    g_aBadRoots = ArrayCreate(32);
    g_aBadMatchType = ArrayCreate(1);
    LoadWords();
...
public plugin_end() {
    ...
    ArrayDestroy(g_aBadRoots);
    ArrayDestroy(g_aBadMatchType);
...
LoadWords() {
    TrieClear(g_tBadWords);
    ArrayClear(g_aBadWords);
    ArrayClear(g_aBadRoots);
    ArrayClear(g_aBadMatchType);
    ...
    while (!feof(f)) {
        ...
        new iMatchType = MATCH_EXACT;
        new iLen = strlen(szLine);
        
        // *kelime* -> MATCH_CONTAINS
        if (iLen > 2 && szLine[0] == '*' && szLine[iLen - 1] == '*') {
            iMatchType = MATCH_CONTAINS;
            szLine[iLen - 1] = '^0';
            copy(szLine, charsmax(szLine), szLine[1]);
            trim(szLine);
        }
        // *kelime -> MATCH_SUFFIX
        else if (iLen > 1 && szLine[0] == '*') {
            iMatchType = MATCH_SUFFIX;
            copy(szLine, charsmax(szLine), szLine[1]);
            trim(szLine);
        }
        // kelime* -> MATCH_PREFIX
        else if (iLen > 1 && szLine[iLen - 1] == '*') {
            iMatchType = MATCH_PREFIX;
            szLine[iLen - 1] = '^0';
            trim(szLine);
        }
        ...
        if (szClean[0] != '^0') {
            if (iMatchType == MATCH_EXACT) {
                TrieSetCell(g_tBadWords, szClean, 1);
                ArrayPushString(g_aBadWords, szClean);
            } else if (iMatchType == MATCH_CONTAINS) {
                new szEntry[64];
                formatex(szEntry, charsmax(szEntry), "*%s*", szClean);
                ArrayPushString(g_aBadWords, szEntry);
            } else if (iMatchType == MATCH_PREFIX) {
                new szEntry[64];
                formatex(szEntry, charsmax(szEntry), "%s*", szClean);
                ArrayPushString(g_aBadWords, szEntry);
            } else if (iMatchType == MATCH_SUFFIX) {
                new szEntry[64];
                formatex(szEntry, charsmax(szEntry), "*%s", szClean);
                ArrayPushString(g_aBadWords, szEntry);
            }
            
            ArrayPushString(g_aBadRoots, szClean);
            ArrayPushCell(g_aBadMatchType, iMatchType);
        }
    }
    
    fclose(f);
    log_amx("[AutoGag] Kelime listesi yuklendi (%d kayit).", ArraySize(g_aBadRoots));
}

public cmd_Say(id) {
    if (!is_user_connected(id)) return PLUGIN_CONTINUE;
    
    // Admin dokunulmazligi korumasi (1 ise yetkililer muaf, 0 ise yetkililer de denetlenir)
    if (get_pcvar_num(g_pCvarImmunity) && access(id, ADMIN_IMMUNITY)) return PLUGIN_CONTINUE;
    ...
    // 1. ADIM: Spaceless (Tum mesaj tek parca) kontrol
    // Bilesik kufurler veya bosluksuz yazimlar icin
    new szGlobalSpaceless[192];
    copy(szGlobalSpaceless, charsmax(szGlobalSpaceless), szMessage);
    CleanWord(szGlobalSpaceless);
    
    if (szGlobalSpaceless[0] != '^0' && !TrieKeyExists(g_tWhitelist, szGlobalSpaceless)) {
        if (CheckWordMatch(szGlobalSpaceless)) {
            bFound = true;
        } else {
            new iSize = ArraySize(g_aBadRoots);
            for (new i = 0; i < iSize; i++) {
                new iMatchType = ArrayGetCell(g_aBadMatchType, i);
                if (iMatchType == MATCH_CONTAINS) {
                    new szRoot[32];
                    ArrayGetString(g_aBadRoots, i, szRoot, charsmax(szRoot));
                    new rLen = strlen(szRoot);
                    if (rLen >= 3 && contain(szGlobalSpaceless, szRoot) != -1) {
                        bFound = true;
                        break;
                    }
                }
    ...
    // 2. ADIM: Kelime kelime ve ardisik 2'li kelime (bigram) kontrolu
    if (!bFound) {
        new szPrevToken[64];
        szPrevToken[0] = '^0';
        ...
        while ((iPos = argparse(szMessage, iPos, szWord, charsmax(szWord))) != -1) {
            new szCleanToken[64];
            copy(szCleanToken, charsmax(szCleanToken), szWord);
            CleanWord(szCleanToken);
            
            // Tekil kelime kontrolu
            if (CheckWordMatch(szWord)) {
                bFound = true;
                break;
            }
            
            // Iki kelimenin birlesimi kontrolu (Orn: "o" + "cocugu" = "ococugu", "orospu" + "evladi" = "orospuevladi")
            if (szPrevToken[0] != '^0' && szCleanToken[0] != '^0') {
                new szCombined[128];
                formatex(szCombined, charsmax(szCombined), "%s%s", szPrevToken, szCleanToken);
                if (CheckWordMatch(szCombined)) {
                    bFound = true;
                    break;
                }
            }
            
            copy(szPrevToken, charsmax(szPrevToken), szCleanToken);
        }
    }
    
    // 3. ADIM: Noktalama veya bosluklarla harf harf ayrilmis kelimelerin tespiti. Orn: "k u f u r" veya "s i k e r i m"
    if (!bFound) {
        ...
        new szLetterAccumulator[64];
        new iAccLen = 0;
        szLetterAccumulator[0] = '^0';
        ...
        while ((iPos = argparse(szPunctspacedMessage, iPos, szWord, charsmax(szWord))) != -1) {
            ...
            new iLen = strlen(szCleanToken);
            if (iLen == 1) {
                if (iAccLen < charsmax(szLetterAccumulator)) {
                    szLetterAccumulator[iAccLen++] = szCleanToken[0];
                    szLetterAccumulator[iAccLen] = '^0';
                }
            } else {
                if (iAccLen > 1 && CheckWordMatch(szLetterAccumulator)) {
                    bFound = true;
                    break;
                }
                szLetterAccumulator[0] = '^0';
                ...
                if (iLen > 0 && CheckWordMatch(szCleanToken)) {
                    bFound = true;
                    break;
                }
            }
        ...
        if (!bFound && iAccLen > 1) {
            if (CheckWordMatch(szLetterAccumulator)) {
                bFound = true;
            }
        }
    }
    ...
    if (bFound) {
        ...
        if (g_iWarnings[id] >= get_pcvar_num(g_pCvarWarnLimit)) {
            ...
            if (g_iOffenses[id] >= 5) {
                iGagTime = 0; // 0 = Kalici / Suresiz Gag
                client_print_color(id, print_team_default, "%sYasakli kelime sinirini astiginiz icin ^3kalici (suresiz) ^1gaglandiniz.", AUTOGAG_TAG);
                formatex(szReason, charsmax(szReason), "Otomatik Gag (Yasakli Kelime 5+ Ihlal - Kalici)");
                log_amx("[AutoGag] %s yasakli kelime nedeniyle KALICI olarak gaglandi. Ihlal: %d", szName, g_iOffenses[id]);
            } else {
                new iDefaultTime = get_pcvar_num(g_pCvarDefaultTime);
                new iShift = min(g_iOffenses[id] - 1, 14);
                iGagTime = iDefaultTime * (1 << iShift);
...
public cmd_AddWord(id, level, cid) {
    ...
    if (f) {
        fprintf(f, "%s^n", szFormattedEntry);
        fclose(f);
        LoadWords();
        console_print(id, "[AutoGag] '%s' kurali basariyla eklendi.", szFormattedEntry);
    } else {
...
bool:ContainsSubsequence(const szText[], const szPattern[], maxGap, maxStart) {
    new tLen = strlen(szText);
    new pLen = strlen(szPattern);
    
    if (tLen < pLen) return false;
    if (TrieKeyExists(g_tWhitelist, szText)) return false;
    
    new searchStartLimit = min(maxStart, tLen - pLen);
    
    for (new start = 0; start <= searchStartLimit; start++) {
        if (szText[start] != szPattern[0]) continue;
        
        new pIndex = 1;
        new lastPos = start;
        new bool:bMatch = true;
        
        while (pIndex < pLen) {
            new targetChar = szPattern[pIndex];
            new nextPos = -1;
            new searchLimit = min(lastPos + 1 + maxGap, tLen - 1);
            
            for (new k = lastPos + 1; k <= searchLimit; k++) {
                if (szText[k] == targetChar) {
                    nextPos = k;
                    break;
                }
            }
...
bool:CheckWordMatch(const szWord[]) {
    ...
    if (szClean[0] == '^0') return false;
    
    // Beyaz liste kontrolu (oncelikli)
    if (TrieKeyExists(g_tWhitelist, szClean)) {
        return false;
    }
    
    // 1. Tam eslesme (Trie O(1))
    if (TrieKeyExists(g_tBadWords, szClean)) {
        return true;
    }
    
    // 2. Dinamik Eslesme (PREFIX, CONTAINS, SUFFIX ve Alt-Dizi / Subsequence)
    new iSize = ArraySize(g_aBadRoots);
    new tLen = strlen(szClean);
    ...
    for (new i = 0; i < iSize; i++) {
        ...
        if (tLen < rLen) continue;
        
        switch (iMatchType) {
            case MATCH_PREFIX: {
                // On-ek eslesmesi (Orn: sik* -> sikerler, siktir)
                if (equal(szClean, szRoot, rLen)) {
                    return true;
                }
                // Baslangicta araya harf girme bypass'i (siokerler, saiker, sxixk)
                if (ContainsSubsequence(szClean, szRoot, (rLen >= 4 ? 2 : 1), 1)) {
                    return true;
                }
            }
            case MATCH_CONTAINS: {
                // Kelimenin herhangi bir yerinde icerme (Orn: *skm* -> anskm, anaskm; *amk* -> yohamk; *siktir* -> hasiktir)
            ...
            case MATCH_SUFFIX: {
                // Son-ek eslesmesi
                if (equal(szClean[tLen - rLen], szRoot)) {
                    return true;
                }
            }
            case MATCH_EXACT: {
